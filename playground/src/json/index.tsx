...
export function parse(
  ...
  onObjectEndExtended?: (object: any) => any
): any {
  ...
  const visitor: JSONVisitor = {
    ...
    onObjectEnd: () => {
      if (onObjectEndExtended) {
        onObjectEndExtended(currentParent);
      }
 ...
 * Therefore always check the errors list to find out if the input was valid.
 */
export function parseWithLines(
  text: string,
  errors: ParseError[] = [],
  options: ParseOptions = ParseOptions.DEFAULT,
  onObjectEndExtended?: (object: any) => any
  ...
  const visitor: JSONVisitor = {
    onObjectBegin: (offset: number) => {
      ...
      Object.defineProperty(object, "__line__", {
        value: [offset],
        enumerable: false,
      });
    ...
    onObjectEnd: (offset: number, tokenLength: number) => {
      currentParent.__line__.push(
        offset + tokenLength + 1 - currentParent.__line__[0]
      );
    ...
    onArrayBegin: (offset: number) => {
      const array: any[] = [];
      Object.defineProperty(array, "__line__", {
        value: [offset],
        enumerable: false,
      });
    ...
    onArrayEnd: (offset: number, tokenLength: number) => {
      currentParent.__line__.push(
        offset + tokenLength + 1 - currentParent.__line__[0]
      );
